...
class classificationsService {
  ...
  static async getAllClassifications(query) {
    ...
    let sql=''
    if(query.deletef==='0'){
      sql=" where child.deletef=0"
    }
    await Sequelize.query(
      "select child.name,child.deletef,child.id,child.parentId,child.created_time ,parent.name as parent_name FROM classifications parent right join classifications child on parent.id=child.parentId or child.parentId=child.id"+sql
